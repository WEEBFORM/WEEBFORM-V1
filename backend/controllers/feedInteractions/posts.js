            ...
            if (images) {
                for (const image of images) {
                    try {
                        const params = {
                            Bucket: process.env.BUCKET_NAME,
                            Key: `uploads/${Date.now()}_${image.originalname}`,
                            Body: image.buffer,
            ...
            if (videos) {
                for (const video of videos) {
                    try {
                        const params = {
                            Bucket: process.env.BUCKET_NAME,
                            Key: `uploads/${Date.now()}_${video.originalname}`,
                            Body: video.buffer,
...
export const userPosts = async(req, res) => {
    authenticateUser(req, res, () => {
        ...
        const q = `SELECT p.*, u.id AS userId, username, full_name, profilePic, COUNT(l.id) AS likesCount
            FROM 
                posts AS p
            JOIN 
                users AS u ON u.id = p.userId
            LEFT JOIN 
                likes AS l ON l.postId = p.id
            GROUP BY 
                p.id, u.id FROM posts AS p LEFT JOIN users AS u ON (u.id = p.userId) WHERE userId = ? ORDER BY createdAt DESC`;
