...
export const getComment = (req, res) => {
    authenticateUser(req, res, () => {
        ...
        const q = `
            SELECT c.*, u.id AS userId, username, full_name, profilePic
            FROM comments AS c
    ...
    authenticateUser(req, res, () => {
        ...
        db.query(q, [commentId, user.id], (err, data) => {
            ...
            if (data.affectedRows > 0) {
                // Invalidate the cache for the associated post
                ...
                res.status(403).json("Can only delete your own comment"); // Use 403 for Forbidden
            }
        });
    });
};