...
export const like = async (req, res) => {
    authenticateUser(req, res, async () => {
        ...
        // Use individual placeholders for userId and communityPostId
        const q = "INSERT INTO likes (userId, communityPostId) VALUES(?, ?)";
        // Pass userId and communityPostId as separate parameters
        const values = [userId, communityPostId];
        try {
...
export const getLikes = async (req, res) => {
    ...
    let cacheKey = `likes:${communityPostId}`;
    ...
    try {
         const cachedData = likeCache.get(cacheKey);
             ...
             if (cachedData) {
                ...
                const q = "SELECT l.*, u.username, u.id AS userId FROM likes AS l JOIN users AS u ON (u.id = l.userId) WHERE l.communityPostId = ?";
