...
export const fetchGroupMessages = async (req, res) => {
    authenticateUser(req, res, async () => {
        ...
        try {
            const query = `
            SELECT gm.id, gm.userId as senderId, gm.chatGroupId, gm.text as message, gm.media, gm.createdAt,
            ...
            db.query(query, [chatGroupId], async (err, results) => {
                if (err) {
            ...
            if (result && result.length > 0) {
                ...
                if (media) {
                    ...
                    await Promise.all(mediaArray.map(async (mediaUrl) => {
                        ...
                        const mediaKey = s3KeyFromUrl(mediaUrl);
                        try {
                            await s3.send(new DeleteObjectCommand({ Bucket: process.env.BUCKET_NAME, Key: mediaKey }));
                        } catch (error) {
